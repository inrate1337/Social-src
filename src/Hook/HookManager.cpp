...
#include "Hooks/ContainerHooks/ContainerSlotHoveredHook.hpp"
...
#include "Hooks/RenderHooks/FogColorHook.hpp"
...
void HookManager::init(bool initLp)
{
    ...
    else
    {
        ...
        ADD_HOOK(FogColorHook);
        ADD_HOOK(D3DHook);
        ADD_HOOK(ContainerScreenControllerHook);
        ADD_HOOK(ContainerSlotHoveredHook);
        ADD_HOOK(MouseHook);
