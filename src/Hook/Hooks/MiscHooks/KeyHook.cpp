...
void KeyHook::onKey(uint32_t key, bool isDown)
{
    ...
    if (key == VK_END && isDown)
    {
        ...
        const bool inChat = (ci && ci->getScreenName() == "chat_screen");
        if (!inChat)
        {
            bool imguiBlocking = false;
            if (hasImGui)
            {
                const ImGuiIO& io = ImGui::GetIO();
                imguiBlocking = io.WantCaptureKeyboard || io.WantTextInput;
            }
    ...
    {
        if (ci->getMouseGrabbed() && module.get() != clickGui) continue;
        if (ci->getScreenName() == "chat_screen") continue;
