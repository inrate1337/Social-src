...
AttributeInstance* Actor::getAttribute(int id)
{
    ...
    if (!component) {
        ...
        if ((nowMs - prev) > 2000 && lastLogMs.compare_exchange_strong(prev, nowMs, std::memory_order_relaxed))
        {
            const uint32_t sup = suppressed.exchange(0, std::memory_order_relaxed);
            if (sup > 0) spdlog::warn("Actor::getAttribute: No AttributesComponent found (suppressed {})", sup);
            else spdlog::warn("Actor::getAttribute: No AttributesComponent found");
        }
        else
        {
