...
void AutoPath::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    if (player == nullptr || !ActorUtils::isActorValid(player))
        return;

    auto actors = ActorUtils::getActorList(false, true);
    std::vector<Actor*> filtered;
    filtered.reserve(actors.size());
    for (auto a : actors)
    {
        if (!a || a == player) continue;
        if (!ActorUtils::isActorValid(a)) continue;
        filtered.push_back(a);
    }
    actors = std::move(filtered);

    std::ranges::sort(actors, [&](Actor* a, Actor* b) -> bool
    {
        glm::vec3* aPos = nullptr;
        glm::vec3* bPos = nullptr;
        glm::vec3* pPos = nullptr;
        if (!TryCallWrapper([&]() { aPos = a->getPos(); })) return false;
        if (!TryCallWrapper([&]() { bPos = b->getPos(); })) return true;
        if (!TryCallWrapper([&]() { pPos = player->getPos(); })) return false;
        if (!aPos || !bPos || !pPos) return false;
        return glm::distance(*aPos, *pPos) < glm::distance(*bPos, *pPos);
    });

    if (actors.empty()) return;

    auto target = actors.at(0);
    if (target == nullptr || !ActorUtils::isActorValid(target)) return;

    glm::vec3* targetPosPtr = nullptr;
    glm::vec3* playerPosPtr = nullptr;
    if (!TryCallWrapper([&]() { targetPosPtr = target->getPos(); }) || !targetPosPtr) return;
    if (!TryCallWrapper([&]() { playerPosPtr = player->getPos(); }) || !playerPosPtr) return;
    Vec3i targetng = Vec3i(targetPosPtr->x, targetPosPtr->y, targetPosPtr->z);
    Vec3i srcng = Vec3i(playerPosPtr->x, playerPosPtr->y, playerPosPtr->z);
    const auto targetPos = *targetPosPtr;
