...
void DeviceSpoof::spoofMboard() {
    ...
    if (sUseCustom && !sCustomDeviceModel.empty())
    {
        DeviceModel = sCustomDeviceModel;
    }
    else if (!editionFaker || !editionFaker->mEnabled)
    {
...
void DeviceSpoof::onConnectionRequestEvent(ConnectionRequestEvent& event)
{
    ...
    if (sUseCustom && !sCustomDeviceId.empty())
    {
        deviceId = sCustomDeviceId;
    }
    else if (!editionFaker || !editionFaker->mEnabled)
    {
    ...
    const int64_t clientRandom = (sUseCustom && sCustomClientRandomId != 0) ? sCustomClientRandomId : StringUtils::generateCID();
    const std::string skinId = (sUseCustom && !sCustomSkinId.empty()) ? sCustomSkinId : ("Custom" + deviceId);
    const std::string selfSigned = (sUseCustom && !sCustomSelfSignedId.empty()) ? sCustomSelfSignedId : StringUtils::generateUUID(7);

    event.mClientRandomId = clientRandom;
    *event.mDeviceId = deviceId;
    *event.mSkinId = skinId;
    *event.mSelfSignedId = selfSigned;
