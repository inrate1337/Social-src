...
void AutoQueue::onPacketInEvent(PacketInEvent& event)
{
    ...
    if (event.mPacket->getId() == PacketID::Text) {
        ...
        const std::string& msg = tp->getText();
        if (msg == "§c§l» §r§c§lGame OVER!" && mQueueOnGameEnd) {
            mQueueForGame = true;
            mLastQueueTime = NOW;
        ...
        if (playerName != "") {
            ...
            if (StringUtils::containsIgnoreCase(msg, "§7has been §cELIMINATED§7!") && StringUtils::startsWith(msg, playerTeam + "§l»") && mQueueOnDeath) {
                mQueueForGame = true;
                mLastQueueTime = NOW;
                NotifyUtils::notify("Queuing for " + mLastGame + "!", 1.f + (mQueueDelay.mValue), Notification::Type::Info);
            }
        ...
        if (StringUtils::containsAnyIgnoreCase(msg, ignored)) {
            event.mCancelled = true;
            return;
        }

        if (StringUtils::containsIgnoreCase(msg, "You are connected to server "))
        {
            // You are connected to server name HUB-SKY9
            std::string game = msg.substr(msg.find("server name ") + 12, msg.length() - 1);
