...
static bool isTridentItemStack(ItemStack* item)
{
    if (!item || !item->valid || item->mCount <= 0) return false;
    if (!item->mItem || *item->mItem == nullptr) return false;
    auto it = item->getItem();
    if (!it) return false;
    return static_cast<int>(it->mItemId) == 579;
}

static std::shared_ptr<MobEquipmentPacket> buildSpoofMobEquipmentPacket(Actor* player, int slot)
{
    if (!player) return {};
    ...
    auto pkt = MinecraftPackets::createPacket<MobEquipmentPacket>();
    pkt->mRuntimeId = player->getRuntimeID();
    pkt->mItem = NetworkItemStackDescriptor(*stack);
    pkt->mSlot = slot;
    pkt->mSelectedSlot = slot;
    pkt->mContainerId = MobEquipmentPacket::ContainerID::Inventory;
    pkt->mSlotByte = static_cast<unsigned char>(slot);
    pkt->mSelectedSlotByte = static_cast<unsigned char>(slot);
    pkt->mContainerIdByte = static_cast<unsigned char>(MobEquipmentPacket::ContainerID::Inventory);
    return pkt;
}

static bool queueSpoofSlot(Actor* player, int slot, bool bypassHook)
{
    auto pkt = buildSpoofMobEquipmentPacket(player, slot);
    if (!pkt) return false;
    PacketUtils::queueSend(pkt, bypassHook);
    return true;
}

static int antiswapnaxui(PlayerInventory* supplies)
{
    ...
    std::vector<int> nonToolSlots;
    emptySlots.reserve(9);
    nonToolSlots.reserve(9);
    ...
    for (int slot = 0; slot < 9; slot++)
    {
        ...
        if (isTridentItemStack(item)) continue;
        if (ItemUtils::hasItemType(item)) continue;
        nonToolSlots.push_back(slot);
    }

    const std::vector<int>* pool = nullptr;
    if (!emptySlots.empty())
        pool = &emptySlots;
    else if (!nonToolSlots.empty())
        pool = &nonToolSlots;
    else
        return -1;

    int index = MathUtils::random(0, static_cast<int>(pool->size()) - 1);
    return (*pool)[index];
}
...
void Disabler::onPacketOutEvent(PacketOutEvent& event)
{
    ...
    else if (mMode.mValue == Mode::Firzmine)
    {
        if (event.mPacket->getId() == PacketID::PlayerAuthInput) {
            if (mFirzmineUseItem.mValue && ImGui::IsMouseDown(1)) return;
            const float cps = mFirzmineSelfCps.mValue;
            const uint64_t delayMs = static_cast<uint64_t>(1000.0f / cps);
            if (NOW - mLastSelfAttack > delayMs) {
                auto supplies = player->getSupplies();
                int slot = antiswapnaxui(supplies);
                if (slot == -1) return;

                int currentSlot = supplies ? supplies->mSelectedSlot : -1;
                bool spoofSlot = currentSlot != -1 && currentSlot != slot;
                bool didSpoof = false;

                if (spoofSlot)
                {
                    didSpoof = queueSpoofSlot(player, slot, true);
                }

                if (!mFirzmineNoSwing.mValue)
                {
                    player->swing();
                }

                auto selfAttack = ActorUtils::createAttackTransaction(player, didSpoof ? slot : -1);
                if (selfAttack)
                {
                    PacketUtils::queueSend(std::reinterpret_pointer_cast<Packet>(selfAttack), true);
                    mLastSelfAttack = NOW;
                }

                if (didSpoof)
                {
                    queueSpoofSlot(player, currentSlot, true);
                }
            }
        }
    }
