...
public:
    ...
    EnumSettingT<Mode> mMode = EnumSettingT<Mode>("Mode", "The mode to use for the disabler.", Mode::Flareon, "Flareon", "Sentinel", "Lifeboat",
        ...
        "SentinelNew",
#endif
        "Firzmine", "Custom", "BDS Prediction");
    ...
    BoolSetting mFirzmineNoSwing = BoolSetting("Firzmine No Swing", "Hide swing animation in Firzmine", true);
    BoolSetting mFirzmineUseItem = BoolSetting("Use Item", "Allow using items while Firzmine is active", false);
    NumberSetting mFirzmineSelfCps = NumberSetting("Self CPS", "Self-attack CPS while Firzmine is active", 3.5f, 1.0f, 20.0f, 0.1f);
    ...
    Disabler() : ModuleBase<Disabler>("Disabler", "Attempts to disable Anti-Cheat checks by exploiting them.", ModuleCategory::Misc, 0, false){
        ...
        addSettings(&mInteract, &mCancel, &mGlide, &mOnGroundSpoof, &mInputSpoof, &mClickPosFix, &mFirzmineNoSwing, &mFirzmineUseItem, &mFirzmineSelfCps);
        ...
        VISIBILITY_CONDITION(mFirzmineNoSwing, mMode.mValue == Mode::Firzmine);
        VISIBILITY_CONDITION(mFirzmineUseItem, mMode.mValue == Mode::Firzmine);
        VISIBILITY_CONDITION(mFirzmineSelfCps, mMode.mValue == Mode::Firzmine);
