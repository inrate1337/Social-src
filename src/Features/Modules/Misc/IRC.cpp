...
{
    auto player = ClientInstance::get()->getLocalPlayer();
    bool isConnected = IrcManager::mClient && IrcManager::mClient->mConnectionState.load() == ConnectionState::Connected;
...
void IRC::onModuleStateChangeEvent(ModuleStateChangeEvent& event)
{
    ...
    if (IrcManager::mClient && IrcManager::mClient->mConnectionState.load() == ConnectionState::Connecting)
    {
...
void IRC::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    IrcManager::setShowNamesInChat(mShowNamesInChat.mValue);
    IrcManager::setAlwaysSendToIrc(mAlwaysSendToIrc.mValue);
    if (IrcManager::mClient && IrcManager::mClient->mConnectionState.load() == ConnectionState::Connecting && IrcManager::mLastConnectAttempt + 15000 < NOW)
    {
        IrcManager::mClient->disconnect(xorstr_("Connect timeout"));
    }
    if (IrcManager::mClient && IrcManager::mLastConnectAttempt + 5000 < NOW && IrcManager::mClient->mConnectionState.load() == ConnectionState::Disconnected)
    {
