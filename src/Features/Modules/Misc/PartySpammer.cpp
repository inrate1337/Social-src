...
void PartySpammer::onPacketInEvent(PacketInEvent& event)
{
    ...
    if (mMode.mValue == Mode::Form) {
        ...
        if (event.mPacket->getId() == PacketID::Text) {
            ...
            const std::string& msg = textPacket->getText();
            if (mHideInviteMessage.mValue && StringUtils::containsIgnoreCase(msg, "Invited") && StringUtils::containsIgnoreCase(msg, "to your current party. They have 1 minute to accept.")) {
