...
float calculateBps(Actor* actor, const glm::vec3& lastPosition) {
    if (!ActorUtils::isActorValid(actor)) return 0.f;
    glm::vec3* posPtr = nullptr;
    if (!TryCallWrapper([&]() { posPtr = actor->getPos(); })) return 0.f;
    if (!posPtr) return 0.f;
    glm::vec3 currentPos = *posPtr;
...
void Anticheat::onBaseTickEvent(BaseTickEvent& event) {
    ...
    for (auto actor : actors) {
        ...
        if (!isPlayer || actor == localPlayer) {
            continue;
        }

        if (playerMap.find(actor) == playerMap.end()) {
            playerMap[actor] = PlayerInfo();
            TryCallWrapper([&]() { playerMap[actor].playerName = actor->getRawName(); });
            glm::vec3* posPtr = nullptr;
            if (!TryCallWrapper([&]() { posPtr = actor->getPos(); }) || !posPtr) continue;
            playerMap[actor].lastPosition = *posPtr;
        }

        auto& playerInfo = playerMap[actor];
        glm::vec3* posPtr = nullptr;
        if (!TryCallWrapper([&]() { posPtr = actor->getPos(); }) || !posPtr) continue;
        glm::vec3 currentPosition = *posPtr;
        glm::vec3 lastPosition = playerInfo.lastPosition;

        ActorRotationComponent* rotationComponent = nullptr;
        if (!TryCallWrapper([&]() { rotationComponent = actor->getActorRotationComponent(); })) rotationComponent = nullptr;
        if (!rotationComponent) continue;
        ...
        TryCallWrapper([&]() { isImmobile = actor->getStatusFlag(ActorFlags::Noai); });
