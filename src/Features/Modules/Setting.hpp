...
public:
    ...
    virtual EnumSettingBase* asEnumSettingBase()
    {
        return nullptr;
    }

    virtual const EnumSettingBase* asEnumSettingBase() const
    {
        return nullptr;
    }
...
{
public:
    EnumSettingBase(std::string name, std::string description)
        : Setting(std::move(name), std::move(description), SettingType::Enum)
    {
    }

    EnumSettingBase* asEnumSettingBase() override
    {
        return this;
    }

    const EnumSettingBase* asEnumSettingBase() const override
    {
        return this;
    }
...
public:
    ...
    std::vector<float> mAnim;

    EnumSetting(std::string name, std::string description, int index, std::vector<std::string> values)
        : EnumSettingBase(std::move(name), std::move(description)), mValue(index), mValues(std::move(values))
    {
        mAnim.resize(mValues.size(), 0.0f);
    }

    template <typename IndexType, typename... Args>
    EnumSetting(std::string name, std::string description, IndexType index, Args... values)
        : EnumSettingBase(std::move(name), std::move(description)), mValue(static_cast<int>(index))
    {
        mValues = { values... };
        mAnim.resize(mValues.size(), 0.0f);
    }
    ...
    const std::vector<std::string>& getValues() const override
    {
        return mValues;
    }
...
template <typename T>
class EnumSettingT : public EnumSettingBase
{
public:
    T mValue;
    std::vector<std::string> mValues;
    ...
    {
        nlohmann::json j = Setting::serialize();
        j["enumValue"] = static_cast<int>(mValue);
        return j;
    }

    template <typename... Args>
    EnumSettingT(std::string name, std::string description, T index, Args... values)
        : EnumSettingBase(std::move(name), std::move(description)), mValue(index)
    {
        mValues = { values... };
        mAnim.resize(mValues.size(), 0.0f);
    }
