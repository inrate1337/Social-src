...
void Arraylist::onRenderEvent(RenderEvent& event)
{
    ...
    for (auto& mod : module)
    {
        ...
        ImColor color = ColorUtils::getGuiAccentColor(pos.y * 2);
    ...
    pos = ImVec2(ImGui::GetIO().DisplaySize.x - 10, 10); 
    ...
    for (auto& mod : module)
    {
        ...
        if (!settingDisplay.empty())
        {
            ...
            drawList->AddText(ImGui::GetFont(), fontSize, textPos, ColorUtils::getUiTextDimColor(1.0f), settingDisplay.c_str());
        }

        if (isHovered)
        {
            drawList->AddRectFilled(ImVec2(rect.x, rect.y), ImVec2(rect.z, rect.w), ColorUtils::getUiTextColor(0.08f));
