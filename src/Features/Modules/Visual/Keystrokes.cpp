...
void Keystrokes::onRenderEvent(RenderEvent& event)
{
    ...
    // Get W center pos (only center x between box)
    {
        ...
        ImColor keyColor = mUseInterfaceColor.mValue ? ColorUtils::getGuiAccentColor(wPos.x + wPos.y * 5) : ColorUtils::getUiTextColor(200.0f / 255.0f);
        keyColor.Value.w *= keyAnimation[0];

        // Draw the key background
        drawList->AddRectFilled(wPos, ImVec2(wPos.x + keySize.x, wPos.y + keySize.y), ColorUtils::getUiCardColor(150.0f / 255.0f), rounding);
        ...
        drawList->AddText(ImGui::GetFont(), size, ImVec2(wPos.x + keySize.x / 2 - textSize.x / 2, wPos.y + keySize.y / 2 - textSize.y / 2), ColorUtils::getUiTextColor(1.0f), "W");
    }
    ...
    // A (second row)
    {
        ...
        ImColor keyColor = mUseInterfaceColor.mValue ? ColorUtils::getGuiAccentColor(aPos.x + aPos.y * 5) : ColorUtils::getUiTextColor(200.0f / 255.0f);
        keyColor.Value.w *= keyAnimation[2];

        // Draw the key background
        drawList->AddRectFilled(aPos, ImVec2(aPos.x + keySize.x, aPos.y + keySize.y), ColorUtils::getUiCardColor(150.0f / 255.0f), rounding);
        ...
        drawList->AddText(ImGui::GetFont(), size, ImVec2(aPos.x + keySize.x / 2 - textSize.x / 2, aPos.y + keySize.y / 2 - textSize.y / 2), ColorUtils::getUiTextColor(1.0f), "A");
    }
    ...
    // S (second row)
    {
        ...
        ImColor keyColor = mUseInterfaceColor.mValue ? ColorUtils::getGuiAccentColor(sPos.x + sPos.y * 5) : ColorUtils::getUiTextColor(200.0f / 255.0f);
        keyColor.Value.w *= keyAnimation[1];

        // Draw the key background
        drawList->AddRectFilled(sPos, ImVec2(sPos.x + keySize.x, sPos.y + keySize.y), ColorUtils::getUiCardColor(150.0f / 255.0f), rounding);
        ...
        drawList->AddText(ImGui::GetFont(), size, ImVec2(sPos.x + keySize.x / 2 - textSize.x / 2, sPos.y + keySize.y / 2 - textSize.y / 2), ColorUtils::getUiTextColor(1.0f), "S");
    }
    ...
    // D (second row)
    {
        ...
        ImColor keyColor = mUseInterfaceColor.mValue ? ColorUtils::getGuiAccentColor(dPos.x + dPos.y * 5) : ColorUtils::getUiTextColor(200.0f / 255.0f);
        keyColor.Value.w *= keyAnimation[3];

        // Draw the key background
        drawList->AddRectFilled(dPos, ImVec2(dPos.x + keySize.x, dPos.y + keySize.y), ColorUtils::getUiCardColor(150.0f / 255.0f), rounding);
        ...
        drawList->AddText(ImGui::GetFont(), size, ImVec2(dPos.x + keySize.x / 2 - textSize.x / 2, dPos.y + keySize.y / 2 - textSize.y / 2), ColorUtils::getUiTextColor(1.0f), "D");
    }
    ...
    // SPACE (third row)
    {
        ...
        ImColor keyColor = mUseInterfaceColor.mValue ? ColorUtils::getGuiAccentColor(spacePos.x + spacePos.y * 5) : ColorUtils::getUiTextColor(200.0f / 255.0f);
        keyColor.Value.w *= keyAnimation[4];

        // Draw the key background
        drawList->AddRectFilled(spacePos, ImVec2(spacePos.x + spaceSize.x, spacePos.y + spaceSize.y), ColorUtils::getUiCardColor(150.0f / 255.0f), rounding);
