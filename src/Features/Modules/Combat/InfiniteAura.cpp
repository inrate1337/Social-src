...
void InfiniteAura::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    if (mNeedsToPathBack)
    {
        ...
        auto packetsBack = pathToPos(mLastPos, *playerPosPtr);
    ...
    {
        Actor* actor = *it;
        if (!actor || !ActorUtils::isActorValid(actor)) {
            ++it;
            continue;
        }
        ...
        if (!playerPosPtr || !actorPosPtr)
        {
            ++it;
            continue;
        }
        auto hitResult = bs->checkRayTrace(*playerPosPtr, *actorPosPtr - PLAYER_HEIGHT_VEC, player);
    ...
    for (auto actor : actors)
    {
        ...
        auto playerPosPtr = player->getPos();
        auto actorPosPtr = actor->getPos();
        if (!playerPosPtr || !actorPosPtr) continue;
        auto packets = pathToPos(*playerPosPtr, *actorPosPtr);
