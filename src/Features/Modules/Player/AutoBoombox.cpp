...
void AutoBoombox::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    if (!Aura::sHasTarget || !auraTarget || !auraTarget->isValid() || auraTarget->isDead() || auraTarget->getHealth() <= 0.0f || !auraTarget->getActorTypeComponent()) return;
    if (!auraTarget->isPlayer()) return;
    ...
    glm::ivec3 placePos = *auraTarget->getPos();
    int side = 0;
    float maxDist = 0;

    for (auto& offset : mPlaceOffsets) {
        glm::vec3 pos = *auraTarget->getPos() + offset;
