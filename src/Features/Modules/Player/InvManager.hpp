...
public:
    ...
    BoolSetting mDumpContainerInfo = BoolSetting("Dump Container Info", "Show full info about container items", false);
    BoolSetting mFindWinnerPotion = BoolSetting("Find Winner Potion", "Search chest for §a§lЗелье Победителя", false);
...
#endif
        addSetting(&mDumpContainerInfo);
        addSetting(&mFindWinnerPotion);
    ...
    bool mShouldDumpContainer = false;
    bool mShouldSearchWinnerPotion = false;
    bool mDumpedThisOpen = false;
    ContainerID mOpenContainerId = static_cast<ContainerID>(-1);
    ContainerType mOpenContainerType = static_cast<ContainerType>(-9);
    ...
    void onContainerScreenTickEvent(class ContainerScreenTickEvent& event);
