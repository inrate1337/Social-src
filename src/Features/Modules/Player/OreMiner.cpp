...
bool OreMiner::isValidBlock(glm::ivec3 blockPos, bool oreOnly, bool exposedOnly, bool usePriority, OrePriority priority) {
    ...
    if (oreOnly) {
        ...
        else if (mAncientDebris && (!usePriority || mAncientDebrisPriority.mValue == priority) && isOre(mAncientDebrisIds, blockId)) IsOre = true; // Added Ancient Debris check
        else if (mObsidian && (!usePriority || mObsidianPriority.mValue == priority) && isOre(mObsidianIds, blockId)) IsOre = true; // Added Obsidian check
