...
bool Nuker::isValidBlock(glm::ivec3 blockPos, bool requireExposed) {
    auto player = ClientInstance::get()->getLocalPlayer();
...
int Nuker::getAreaSize() const
{
    switch (mAreaSize.mValue)
    {
    case 1: return 3;
    case 2: return 5;
    case 3: return 7;
...
int Nuker::getDepth() const
{
    int depth = static_cast<int>(std::round(mDepth.mValue));
    if (depth < 1) depth = 1;
    return depth;
}

void Nuker::destroyArea(glm::ivec3 blockPos, int face)
{
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player) return;
    ...
    const int size = getAreaSize();
    const int depth = getDepth();
    if (size <= 1 && depth <= 1)
    {
        Block* block = source->getBlock(blockPos);
        if (!block) return;
        if (!isValidBlock(blockPos, true)) return;
        if (mBlockType.mValue == BlockType::Specified && block->getmLegacy()->getmName() != specifiedBlockID) return;
        if (mSwing.mValue) player->swing();
        BlockUtils::destroyBlock(blockPos, face, mInfiniteDurability.mValue);
        return;
    }

    const std::array<glm::ivec3, 6> faceOffsets = {
        glm::ivec3(0, -1, 0),
        glm::ivec3(0, 1, 0),
        glm::ivec3(0, 0, -1),
        glm::ivec3(0, 0, 1),
    ...
    glm::ivec3 normal = {};
    if (0 <= face && face < 6) normal = -faceOffsets[face];

    glm::ivec3 axis1 = {};
    glm::ivec3 axis2 = {};
    if (face == 0 || face == 1)
    {
        axis1 = glm::ivec3(1, 0, 0);
        axis2 = glm::ivec3(0, 0, 1);
    }
    else if (face == 2 || face == 3)
    {
        axis1 = glm::ivec3(1, 0, 0);
        axis2 = glm::ivec3(0, 1, 0);
    }
    else
    {
        axis1 = glm::ivec3(0, 1, 0);
        axis2 = glm::ivec3(0, 0, 1);
    }
    ...
    int min = -half;
    int max = half;
    if (size % 2 == 0) min = -half + 1;

    if (mSwing.mValue) player->swing();

    for (int d = 0; d < depth; d++)
    {
        glm::ivec3 base = blockPos + (normal * d);
        for (int u = min; u <= max; u++)
        {
            for (int v = min; v <= max; v++)
            {
                glm::ivec3 pos = base + (axis1 * u) + (axis2 * v);
                Block* block = source->getBlock(pos);
                if (!block) continue;
                if (!isValidBlock(pos, false)) continue;
                if (mBlockType.mValue == BlockType::Specified && block->getmLegacy()->getmName() != specifiedBlockID) continue;
                BlockUtils::destroyBlock(pos, face, mInfiniteDurability.mValue);
            }
        }
    }
}
...
void Nuker::queueBlock(glm::ivec3 blockPos) {
    ...
    if (mInstaBreak) destroyArea(blockPos, mCurrentBlockFace);
...
void Nuker::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    if (isValidBlock(mCurrentBlockPos)) { // Check if current block is valid
        ...
        if (mDestroySpeed.mValue <= mBreakingProgress) {
            ...
            if (mSwing.mValue) player->swing();
            destroyArea(mCurrentBlockPos, exposedFace);
