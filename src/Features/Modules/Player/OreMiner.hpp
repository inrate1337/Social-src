...
public:
    ...
    // Для Ancient Debris
    BoolSetting mAncientDebris = BoolSetting("Ancient Debris", "Destroy ancient debris ore", false);
    EnumSettingT<OrePriority> mAncientDebrisPriority = EnumSettingT<OrePriority>("Ancient Debris Priority", "The priority for ancient debris ore", OrePriority::Medium, "High", "Medium", "Low");

    // Для Obsidian
    BoolSetting mObsidian = BoolSetting("Obsidian", "Destroy obsidian ore", false);
    EnumSettingT<OrePriority> mObsidianPriority = EnumSettingT<OrePriority>("Obsidian Priority", "The priority for obsidian ore", OrePriority::Medium, "High", "Medium", "Low");
    ...
    OreMiner() : ModuleBase("OreMiner", "Automatically breaks ores", ModuleCategory::Player, 0, false) {
        ...
        // Добавлено для Ancient Debris
        addSetting(&mAncientDebris);
        addSetting(&mAncientDebrisPriority);

        // Добавлено для Obsidian
        addSetting(&mObsidian);
        addSetting(&mObsidianPriority);
        ...
        // Добавлено для Ancient Debris
        VISIBILITY_CONDITION(mAncientDebrisPriority, mAncientDebris.mValue);

        // Добавлено для Obsidian
        VISIBILITY_CONDITION(mObsidianPriority, mObsidian.mValue);
    ...
    // Добавлены ID для Ancient Debris
    std::vector<int> mAncientDebrisIds = {
        526, // ID для Ancient Debris
    };

    // Добавлены ID для Obsidian
    std::vector<int> mObsidianIds = {
        49, // ID для Obsidian
    };
