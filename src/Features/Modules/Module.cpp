...
{
    Interface* interfaceMod = nullptr;
    if (gFeatureManager && gFeatureManager->mModuleManager)
    {
        interfaceMod = gFeatureManager->mModuleManager->getModule<Interface>();
    }

    NamingStyle style = NamingStyle::NormalSpaced;
    if (interfaceMod) style = static_cast<NamingStyle>(interfaceMod->mNamingStyle.mValue);
...
nlohmann::json Module::serialize()
{
    ...
    {
        j["settings"].push_back(setting->serialize());
    }
    return j;
}
