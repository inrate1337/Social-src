...
void ImRenderUtils::addBlurAlpha(const ImVec4& pos, float strength, float alpha, float radius, ImDrawList* list, bool optimized)
{
    if (!ImGui::GetCurrentContext())
        return;

    if (optimized) D2D::addBlurOptimizedAlpha(list, strength, alpha, pos, radius);
    else D2D::addBlurAlpha(list, strength, alpha, pos, radius);
}

void ImRenderUtils::drawText(ImVec2 pos, const std::string& textStr, const ImColor& color, float textSize, float alpha,
    bool shadow, int index, ImDrawList* d, int pivot)
{
    if (!ImGui::GetCurrentContext())
        return;
    ImFont* font = ImGui::GetFont();

    std::string widthStr = textStr;
    float textWidth = getTextWidth(&widthStr, textSize);
    ImVec2 textPos = ImVec2(
        pivot == 1 ? (pos.x - textWidth * 0.5f) : (pivot == 2 ? (pos.x - textWidth) : pos.x),
        pos.y
    );
    (void)shadow;
...
void ImRenderUtils::drawRectGlow(ImVec4 rect, const ImColor& color, float alpha, float verticalOffset, float radiusMultiplier, float thickness, ImDrawList* list)
{
    if (!ImGui::GetCurrentContext())
        return;
    ...
    float baseRadius = minDim * 0.5f * radiusMultiplier;

    ImVec2 center = ImVec2(
        rect.x + width * 0.5f,
        rect.y + height * 0.5f - verticalOffset
    );
    //penis
    ImVec2 offset = ImVec2(0.f, 0.f);
    float outerRadius = baseRadius * 1.6f;
    float midRadius   = baseRadius * 1.1f;
    float innerRadius = baseRadius * 0.85f;
    //hex penis
    ImColor bottomColor = ImColor(
        0x32 / 255.0f,
        0x34 / 255.0f,
        0x4A / 255.0f,
        alpha * 1.10f
    );
    ImColor midColor = ImColor(
        0x42 / 255.0f,
        0x46 / 255.0f,
        0x66 / 255.0f,
        alpha * 1.35f
    );
    ImColor topColor = ImColor(
        0x5A / 255.0f,
        0x5E / 255.0f,
        0x82 / 255.0f,
        alpha * 1.60f
    );
    list->AddShadowCircle(center, outerRadius, bottomColor, thickness * 2.2f, offset, 0, outerRadius);
    list->AddShadowCircle(center, midRadius,   midColor,    thickness * 1.7f, offset, 0, midRadius);
    list->AddShadowCircle(center, innerRadius, topColor,    thickness * 1.1f, offset, 0, innerRadius);
}
...
void ImRenderUtils::drawCheckMark(ImVec2 pos, float size, const ImColor& color, float alpha, float thickness)
{
	...
	float sz = size * 8.0f;
	float third = sz / 3.0f;
	float thicknessPx = thickness > 0.0f ? thickness : ImMax(sz / 5.0f, 1.0f);
	ImVec2 topLeft = ImVec2(pos.x - sz * 0.5f, pos.y - sz * 0.5f);

	ImVec2 p1 = ImVec2(topLeft.x + third * 0.5f, topLeft.y + third * 1.5f);
	ImVec2 p2 = ImVec2(topLeft.x + third * 1.5f, topLeft.y + third * 2.5f);
	ImVec2 p3 = ImVec2(topLeft.x + third * 3.0f, topLeft.y + third * 0.5f);

	ImU32 col = ImColor(color.Value.x, color.Value.y, color.Value.z, alpha);

	list->PathLineTo(p1);
	list->PathLineTo(p2);
	list->PathLineTo(p3);
	list->PathStroke(col, 0, thicknessPx);
}
