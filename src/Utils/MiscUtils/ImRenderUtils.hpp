...
public:
	...
	static void addBlurAlpha(const ImVec4& pos, float strength, float alpha, float radius = 0.f, ImDrawList* list = ImGui::GetBackgroundDrawList(), bool optimized = false);
	static void drawText(ImVec2 pos, const std::string& textStr, const ImColor& color, float textSize, float alpha, bool shadow = false, int index = 0, ImDrawList* d = ImGui::GetBackgroundDrawList(), int pivot = 0);
	...
	static void drawRectGlow(ImVec4 rect, const ImColor& color, float alpha, float verticalOffset = 0.0f, float radiusMultiplier = 1.0f, float thickness = 100.0f, ImDrawList* list = ImGui::GetBackgroundDrawList());
