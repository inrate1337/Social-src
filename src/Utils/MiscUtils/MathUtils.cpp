...
float MathUtils::animate(float endPoint, float current, float speed) { // Animate the position
    if (!std::isfinite(endPoint)) return current;
    if (!std::isfinite(current)) return endPoint;
    if (!std::isfinite(speed) || speed <= 0.0f) return current;

    float alpha = 1.0f - std::exp(-speed);
    alpha = MathUtils::clamp(alpha, 0.0f, 1.0f);
    return current + (endPoint - current) * alpha;
}
...
std::vector<glm::vec2> MathUtils::getBoxPoints(const AABB& aabb) {
    ...
    for (const auto& wp : worldPoints) {
        glm::vec2 result = {0, 0};
        if (!corrected.OWorldToScreen(origin, wp, result, fov, displaySize)) continue;
        if (result == glm::vec2(FLT_MAX, FLT_MAX)) continue;
