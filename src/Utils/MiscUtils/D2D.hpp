...
struct BlurCallbackData {
    ...
    BlurCallbackData(float strength, float opacity, float rounding, std::optional<ImVec4> clipRect = std::optional<ImVec4>()) {
        this->strength = strength;
...
public:
    ...
    static bool addBlurAlpha(ImDrawList* drawList, float strength, float opacity, std::optional<ImVec4> clipRect, float rounding);
    ...
    static bool addBlurOptimizedAlpha(ImDrawList* drawList, float strength, float opacity, std::optional<ImVec4> clipRect, float rounding);
};
